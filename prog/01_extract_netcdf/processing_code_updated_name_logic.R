...
if(country.id!='NUTS'){ids = name.lookup[,which(colnames(name.lookup)==paste0('GID_',space.res))]}
...
for(date in dates){
    ...
    if(file.exists(raster.current)){
        ...
        analysis.dummy = cbind(analysis.dummy,names,ids)
        weighted.area.national = rbind(weighted.area.national,analysis.dummy)

        weighted.area.national = weighted.area.national[,c(3,1,5,4,2)] # was c(3,1,4,2)
...
names(weighted.area.national.total)[3] = paste0('GID_',space.res)
names(weighted.area.national.total)[4] = paste0('NAME_',space.res)
